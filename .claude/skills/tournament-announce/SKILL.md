...
- 開催中・3日以内の **大型大会**があれば1行添える（Step 1.6）
...
今夜のデイリーとは別に、**開催中 or 3日以内に開催される大型大会**があれば告知に1行添える。毎回この読み取りを回す:

```bash
cd server && npx tsx scripts/ops/upcoming-specials.ts --prod   # 読み取りのみ。開催中 + 3日以内
```

- 出力は候補1件につき1行のJSON（`name` / `startJst` / `daysUntil` / `status` / `reason`）、最終行が `specials=none` または `specials=N`
- `specials=none` なら**何も足さない**。無理に大会を探したり、過去の大会を持ち出したりしない
- 判定はデイリーの曜日プラン（`server/src/modules/tournament/weeklySchedule.ts`）との差分。名前が違う（`reason=name`）か、バイイン・スタック・定員が大きい（`reason=scale`）か、今まさに進行中（`reason=ongoing`）のものだけが出る
- 今夜のデイリー自体は必ず除外されるので、**Step 2 のバリアント文言と重複させない**（今夜の 5-Card などを「大型大会」として二重に書かない）

**触れ方**（見つかったときだけ）:

| 状況 | 触れ方 |
|---|---|
| `reason=ongoing`（開催中） | 「◯◯が今まさに進行中」と1行。今夜の告知が主役なので、詳細（残り人数・賞金）は書かない |
| `daysUntil=0`〜`1` | 大会名と開始日時（`startJst`）を明記して予告する |
| `daysUntil=2`〜`3` | 「M/D に ◯◯があります」程度の軽い予告。煽らない |

- **本文で足すのは1行だけ**。今夜のトナメの告知が主役で、大型大会は添え物
- 大会名・開始時刻は**スクリプト出力の値だけ**を使う（記憶や過去ツイートからコピーしない）
- 賞金額・参加条件など、出力にない情報は書かない
- 3案すべてに入れる必要はない。**1〜2案に入れて切り口を分ける**（例: 案1は今夜に集中、案2で予告）
- URL は既存どおり `https://baby-plo.app` の1本のみ。大会用の別URLは足さない
...
- Step 1.6 で大型大会が見つかった日は、**1〜2案に1行だけ**添える（見つからない日は足さない）
