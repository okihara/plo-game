...
**シーズン表記は必須。** タイトルの冒頭に必ず現行シーズン番号（例:「シーズン2」）を入れる。複数シーズン制を前提にしたランキングなので、どの期間の集計かを明示する。番号をスキル本文にハードコードせず、次のいずれかで**毎回確認**する:
...
## 参考: 望ましい出力例（架空・シーズン番号は現行のものに置き換える）

```
【シーズン<N> RPランキング更新】BabyPLO Holiday 4/19 終了後
