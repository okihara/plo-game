...
it('リエントリー成功でチップがリセットされプライズプールが増える', () => {
  ...
  describe('getClientState', () => {
    ...
    it('スタックが変わらない連続呼び出しでは bubbleFactors を再計算しない', () => {
      const tournament = new TournamentInstance(io, createTestConfig());
      startAndEnterNPlayers(tournament, 3);

      const first = tournament.getClientState().bubbleFactors;
      const second = tournament.getClientState().bubbleFactors;

      expect(first).toBeDefined();
      // 同じスタック状態ならキャッシュヒットで同一オブジェクト参照を返す
      expect(second).toBe(first);
    });

    it('スタックが変わると bubbleFactors を再計算する', () => {
      const tournament = new TournamentInstance(io, createTestConfig());
      startAndEnterNPlayers(tournament, 3);

      const first = tournament.getClientState().bubbleFactors;
      // 1 人のチップを書き換える（バスト直前のスタック変動を模す）
      const players = (tournament as any).players as Map<string, { chips: number }>;
      const firstPlayer = players.values().next().value!;
      firstPlayer.chips = firstPlayer.chips + 100;
  ...
  describe('リエントリー', () => {
    ...
    it('enterPlayer: リエントリー込み総エントリー数でプライズを再計算する', () => {
      const tournament = createReentryTournament();
      // 13 人で開始すると 1 入賞、14 人目で 2 入賞に切り替わる（floor(n*15%)）。
      const { odIds } = startAndEnterNPlayers(tournament, 13);

      expect(tournament.getClientState().payoutStructure).toEqual([
        { position: 1, amount: 1300 },
      ]);

      simulateBust(tournament, odIds[0], 100);
      const result = tournament.enterPlayer(odIds[0], 'Player 0', createMockSocket());
      expect(result.success).toBe(true);
      expect(tournament.getTotalEntries()).toBe(14);
      // 14 entries → 2 paid → [70, 30] of 1400
      expect(tournament.getClientState().payoutStructure).toEqual([
        { position: 1, amount: 980 },
        { position: 2, amount: 420 },
      ]);
    });
    ...
    it('GUEST role はリエントリー上限を超えても許可される', () => {
      const tournament = createReentryTournament({ maxReentries: 1 });
      tournament.start();
      // 比較対象として PLAYER role の参加者を入れて卓を成立させる
      tournament.enterPlayer('player_1', 'Player 1', createMockSocket());
      tournament.enterPlayer('player_2', 'Player 2', createMockSocket());
      // GUEST 参加
      tournament.enterPlayer('guest_0', 'Guest 0', createMockSocket(), { role: 'GUEST' });

      // 1回目バスト → リエントリー成功
      simulateBust(tournament, 'guest_0', 100);
      expect(tournament.canReenter('guest_0')).toBe(true);
      const r1 = tournament.enterPlayer('guest_0', 'Guest 0', createMockSocket());
      expect(r1.success).toBe(true);

      // maxReentries=1 を超えた2回目もリエントリー可能
      simulateBust(tournament, 'guest_0', 100);
      expect(tournament.canReenter('guest_0')).toBe(true);
      const r2 = tournament.enterPlayer('guest_0', 'Guest 0', createMockSocket());
      expect(r2.success).toBe(true);
      expect(tournament.getPlayer('guest_0')?.reentryCount).toBe(2);
    });

    it('GUEST role でも allowReentry=false なら拒否される', () => {
      const tournament = new TournamentInstance(io, createTestConfig({
        allowReentry: false,
      }));
      tournament.start();
      ...
      simulateBust(tournament, 'guest_0', 100);
      expect(tournament.canReenter('guest_0')).toBe(false);
      const result = tournament.enterPlayer('guest_0', 'Guest 0', createMockSocket());
      expect(result.success).toBe(false);
      expect(result.error).toContain('リエントリー不可');
    });

    it('GUEST role でも reentryDeadlineLevel を超えたら拒否される', () => {
      const tournament = createReentryTournament({
        reentryDeadlineLevel: 1,
        maxReentries: 1,
      });
      tournament.start();
      tournament.enterPlayer('player_1', 'Player 1', createMockSocket());
      ...
      const result = tournament.enterPlayer('guest_0', 'Guest 0', createMockSocket());
      expect(result.success).toBe(false);
      expect(result.error).toContain('期限');
    });

    it('role 未指定の enterPlayer は PLAYER として扱われ上限が適用される', () => {
      const tournament = createReentryTournament({ maxReentries: 1 });
      const { odIds } = startAndEnterNPlayers(tournament, 3);

      // ヘルパー経由は role 未指定 → PLAYER が既定
      expect(tournament.getPlayer(odIds[0])?.role).toBe('PLAYER');

      // 1回目はリエントリー成功
      simulateBust(tournament, odIds[0], 100);
      const r1 = tournament.enterPlayer(odIds[0], 'Player 0', createMockSocket());
      expect(r1.success).toBe(true);

      // 2回目は上限により拒否（PLAYER として扱われている証拠）
      simulateBust(tournament, odIds[0], 100);
      expect(tournament.canReenter(odIds[0])).toBe(false);
      const r2 = tournament.enterPlayer(odIds[0], 'Player 0', createMockSocket());
