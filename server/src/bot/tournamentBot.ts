...
import { initSocketErrorLog, getSocketErrorLogPath } from './socketErrorLogger.js';
...
const ALLIN_MODE = process.env.ALLIN_MODE === 'true'; // 全ボットがオールインに向かう（EV計算経路の検証用）
const NO_REENTRY = process.env.NO_REENTRY === 'true'; // 脱落時にリエントリーを試行しない（denied ログ抑制）
const SOCKET_ERROR_LOG = process.env.SOCKET_ERROR_LOG || ''; // 指定パスにソケットエラーのみを書き出す
const GAME_VARIANT = process.env.GAME_VARIANT || 'plo6';

if (SOCKET_ERROR_LOG) {
  initSocketErrorLog(SOCKET_ERROR_LOG);
}
...
async function main(): Promise<void> {
  ...
  console.log(`All-in mode: ${ALLIN_MODE}`);
  console.log(`No reentry:  ${NO_REENTRY}`);
  if (SOCKET_ERROR_LOG) console.log(`Err log:     ${getSocketErrorLogPath()}`);
  ...
  const manager = new TournamentBotManager({
    ...
    allInMode: ALLIN_MODE,
    noReentry: NO_REENTRY,
  });
