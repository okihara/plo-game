...
| 18:00–21:30 | `announce` | サーバー側 LLM 生成の告知 DRAFT を自動投稿。19:00 までに DRAFT が無ければ定型文フォールバックで直接投稿 |
| 22:00–22:45 | `start` | 「スタートしました＋レイトレジ締切」定型文を直接投稿 |
| 22:15–22:38 | `progress` | 「現在Nエントリー＋締切」定型文を直接投稿（エントリー2未満はスキップ） |
| 終了検知後 | `result` | サーバー側 LLM 生成の結果 DRAFT を自動投稿（生成失敗は1回だけ再生成→なお失敗で通知） |
...
- `TWEET_SCHEDULER_ENABLED=true`（ANNOUNCE/RESULT の LLM 生成に必要）
- `ANTHROPIC_API_KEY` 設定済み
- 本リポジトリの変更（`plo_hilo` 告知画像・`specialNote` 対応）をデプロイ済みであること
...
- `FAILED` になったツイートは**自動再投稿しない**（二重投稿防止）。X を確認のうえ `/admin/tweets` から手動対応
- 生成の失敗はサーバー側で Sentry にも飛ぶ（既存機構）
