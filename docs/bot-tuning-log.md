...
### 仮説
ベスト10 vs ワースト10 で **WSD 差が 27.6pt** (69.2% vs 41.6%) と顕著、かつワースト勢の **Cbet 頻度が逆に高い** (15.6% vs 8.6%)。弱い手で cbet → 呼ばれて showdown 負け、というラインで漏れていると推定。OOP (SB/BB) の cbet は構造的に -EV が大きいので、まず SB/BB の cbet 頻度に控えめなペナルティを入れて検証。

### 観測（集団・直近24h, 24,659 ハンド, hands>=100 の 261 bot）

- 全体: Bot 総損益 -44,303 / Human +17,220（昨日と概ね同水準）
- ベスト10 vs ワースト10 (avg):
  - WSD 69.2% vs 41.6% (**+27.6**)
  - Cbet 8.6 vs 15.6 (**-6.9**, ワースト勢の方が cbet 多)
  - 4Bet 6.6 vs 1.6, AFq 58.5 vs 55.7, foldTo3Bet 49.9 vs 45.2
- Personality 別 中央 BB/100:
  - TatsuyaN: -17.8 (5/10: -21.3)
  - YuHayashi: -11.5 (5/10: -23.6 → 大幅改善)
  - yuna0312: -21.6 (5/10: -16.7 → やや悪化)
- ポジション別中央 BB/100: BB **-67.3**, SB -23.2, CO **-20.2** (前日 +5.0 から急回帰), BTN +2.8, MP 0.0, EP +1.4
- BB position は 5/10 -55 → 今日 -67 と変動あり。前日入れた `getPositionBonus('BB')` -0.05→-0.07 の効果はまだ評価中

### 変更
- `server/src/shared/logic/ai/postflopStrategy.ts`
  - `evaluateCbet` 内に OOP 補正を追加: position が `'SB'` または `'BB'` のとき `cbetChance *= 0.85` (約 15% 減)
- 効果: SB/BB が preflop アグレッサーになった場合のフロップ cbet 頻度のみ控えめ化。マルチウェイ補正やボード補正、ハンド強度補正には触らない
- 影響範囲: 全 bot 382 体（全 personality 共通）。ただし発火するのは SB/BB が preflop アグレッサーとなる比較的稀なシナリオのみ

### PR
（後で追記）

### 評価予定
2026-05-13 以降のレポートで全体損益と SB/BB ポジションの BB/100 推移を確認。worst10 の Cbet 率と WSD が改善傾向ならば成功。逆に SB/BB が悪化していたら revert。
