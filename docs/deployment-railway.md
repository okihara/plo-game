...
| `CLIENT_URL` | `https://baby-plo.app` | デプロイ先の正規 URL（OGP / 既定リダイレクト先） |
| `CLIENT_URL_ALIASES` | (未設定) | ドメイン移行時など、追加で許可したいオリジンをカンマ区切りで設定する。CORS / Socket.io / OAuth コールバックの許可ホストに加わる。常用しない |
...
5. `https://baby-plo.app/health`（カスタムドメイン未設定なら `https://<app>.up.railway.app/health`）でヘルスチェックを確認する。

## カスタムドメイン (`baby-plo.app`)

本番では `baby-plo.app` をカスタムドメインとして接続している。新規環境で再現する場合の手順:

1. **Railway 側でドメインを追加**
   - Web サービスの **Settings → Networking → Custom Domains** で `baby-plo.app` を追加する。
...
2. **DNS レコードを設定**
   - レジストラ（または DNS プロバイダ）の管理画面で以下を登録する:
     - `baby-plo.app` → CNAME / ALIAS / ANAME で Railway のターゲットへ
   - 反映には数分〜数時間かかる。Railway 側のドメイン状態が **Active** になるまで待つ。
3. **TLS 証明書**
   - Railway が Let's Encrypt で自動発行する。発行完了まで数分待つ。
4. **環境変数を更新**
   - `CLIENT_URL` を `https://baby-plo.app` に設定し直してサービスを再デプロイする（CORS / Cookie / OAuth コールバック生成の正規 URL）。
   - 別ドメインからの移行が必要なときだけ `CLIENT_URL_ALIASES` に旧オリジンをカンマ区切りで設定する。許可リストに含まれるホスト宛のアクセスについて CORS / Socket.io / OAuth コールバックを動的に許可するため、新旧どちらの URL からでもログインしてプレイできる。完了したら `CLIENT_URL_ALIASES` を削除する。
5. **Twitter Developer Portal を更新**
   - OAuth アプリの **Callback URL** を `https://baby-plo.app/api/auth/twitter/callback` に設定する。
   - **Website URL** も `https://baby-plo.app` に更新する。
6. **動作確認**
   - `https://baby-plo.app/health` が 200 を返す。
