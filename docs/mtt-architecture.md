...
| `tournament:register` | `{ tournamentId }` | 参加登録（DB でバイイン控除） |
| `tournament:unregister` | `{ tournamentId }` | 登録解除（`registering` 中のみ） |
| `tournament:reenter` | `{ tournamentId }` | リエントリー（`eliminated` のみ） |
...
| イベント | ペイロード | トリガー |
|---------|-----------|---------|
| `tournament:list` | `{ tournaments: TournamentLobbyInfo[] }` | `tournament:list` リクエスト時 |
...
ハンド完了ごとに `checkAndExecuteBalance()` で調整:

1. **テーブル破壊**: 全プレイヤーが `(テーブル数-1) × PLAYERS_PER_TABLE` 以下なら最少テーブルを解散
2. **人数調整**: 最多テーブルと最少テーブルの差が 2 以上なら移動
3. **ハンド中ガード**: 移動元がハンド中なら `pendingMoves` に蓄積し、ハンド完了後に実行
