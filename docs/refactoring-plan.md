...
> **✅ 2026-07 完了**: 2-1〜2-4 は段階的抽出ではなく「1本の進行コア + バリアント記述子」への
> 再構成として一括実施した（`server/src/shared/logic/engine/`、設計と検証は
> [engine-core-variants.md](engine-core-variants.md)）。旧6エンジンは公開APIを維持した薄い
> 委譲層になり、既存テスト群 + 新旧差分テスト（13バリアント×2,000ハンド完全一致）で
> 挙動不変を確認済み。2-5（cpuAI レガシー削除）のみ未着手。
