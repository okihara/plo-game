...
export interface PokerStarsHandAction {
  seatIndex: number;
  odId?: string;
  odName: string;
  action: string;
...
export interface PokerStarsHandInput {
  id: string;
  handNumber: number;
  blinds: string;
...
function actionLine(name: string, action: string, amount: number): string {
  switch (action) {
    ...
    case 'allin':
      return `${name}: raises to ${amount} and is all-in`;
    default:
      return `${name}: ${action}${amount > 0 ? ` ${amount}` : ''}`;
  }
}
...
export function toPokerStarsHandText(hand: PokerStarsHandInput): string {
  ...
  const byStreet: Record<StreetKey, PokerStarsHandAction[]> = {
    preflop: [],
    flop: [],
    turn: [],
    river: [],
